...
/**
 * Text → exact base units.
 *
 * Returns `null` rather than throwing or coercing, because every caller is a
 * form field that must show the user *why* their input was rejected. Silently
 * rounding "10.1234567" down to 6 decimals would change the number the user
 * believes they are committing, so excess precision is refused outright.
 */
export function parseTokenAmount(input: string, decimals: number): bigint | null {
  const text = input.trim();
  if (!text) return null;
  // Reject exponent notation, signs and stray characters before ethers sees
  // them — `parseUnits` accepts some of these and they are never intended.
