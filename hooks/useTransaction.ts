...
/** Re-exported so callers building an intent import one type, not two. */
export type TxFact = Fact;
...
export function useTransaction(options: UseTransactionOptions = {}) {
  ...
  // Held in a ref so `confirm` can stay referentially stable while still
  // calling the latest callback. Assigned in an effect rather than during
  // render — mutating a ref while rendering is not safe under concurrent
  // rendering, where a render may be discarded before it commits.
